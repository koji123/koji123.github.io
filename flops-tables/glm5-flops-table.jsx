...
const D=6144,H=64,QR=2048,KR=512;
const Dn=192,Dr=64,Dv=256;
const NE=256,KE=8,MI=2048,SI=12288;
...
function build(S,Skv,isDec,TP,EP){
  ...
  const o=[];
  const tpr=Sq*KE/EP; // avg tokens per EP rank
  ...
  o.push({s:"Expert×8",t:"M",
    ...
    par:`EP÷${EP}`,comm:"",cs:0});
  o.push({s:"Expert×8",t:"E",
    op:`重み付き合成: ${(Sq/EP).toFixed(0)}tok × Σ(w×expert_out)`,
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#08080e,#0e0e1c,#0a0a14)",color:"#e0e0ec",fontFamily:"'IBM Plex Sans','Noto Sans JP',system-ui,sans-serif",padding:"14px 8px"}}>
      ...
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:10}}>
          <h1 style={{fontSize:18,fontWeight:700,color:"#f0f0f5",margin:0}}>GLM-5 — 1層 FLOPS・通信 (全て1GPUあたり)</h1>
          <p style={{fontSize:10,color:"#666",margin:"3px 0 0"}}>演算名+行列サイズ | FLOPS/GPU=実演算量 | 送信/GPU=各GPUの送信量</p>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:5,marginBottom:9}}>
          {[
            {l:"FLOPS/GPU/層",v:fmt(gpuF),u:`×78=${fmt(gpuF*78)}`,c:"#e0e0f0"},
            {l:"冗長(replicated)",v:fmt(replF),u:pct(replF,gpuF),c:"#ffcc44"},
            {l:"分割計算",v:fmt(gpuF-replF),u:pct(gpuF-replF,gpuF),c:"#66bbff"},
            {l:"送信/GPU/層",v:fB(gpuSend),u:`×78=${fB(gpuSend*78)}`,c:"#ffaa66"},
            {l:"AR送信(TP)",v:fB(arS_),u:"×2回/層",c:"#66bbff"},
        ...
        <div style={{marginTop:7,padding:"6px 9px",background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.04)",borderRadius:5,fontSize:9,color:"#666",lineHeight:1.5}}>
          <span style={{color:"#6BB3FF"}}>MM</span>=行列積(2MKN) <span style={{color:"#FFBB66"}}>El</span>=要素演算 <span style={{color:"#ffcc44"}}>★replicated</span>=冗長計算 <span style={{color:"#66bbff"}}>AR</span>=AllReduce <span style={{color:"#cc66ff"}}>A2A</span>=All-to-All |
          行列サイズはTP/EP分割後の1GPU分。Expert FLOPSはtoken均等分配仮定。BF16前提。
        </div>
      </div>
    </div>
  );
